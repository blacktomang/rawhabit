...
export class AiService {
  ...
  async coach(
    ...
    if (!this.client)
      return {
        result: this.fallbackCoach(transcript, template),
        mode: "fallback" as const,
      };
    try {
      ...
      return {
        result: {
          riskLevel: result.riskLevel as AICoachResponse["riskLevel"],
          caption: result.caption.slice(0, 140),
          coachMessage: result.coachMessage,
          suggestedAction: result.suggestedAction || template.strategyRules[0],
        },
        mode: "live" as const,
      };
    } catch (cause) {
  ...
  private fallbackCoach(
    ...
    return {
      riskLevel: "low",
      caption:
        transcript.slice(0, 135) || "An honest check-in, one day at a time.",
      coachMessage:
        "You showed up for the hard moment. Keep your next step small and specific.",
      suggestedAction: template.strategyRules[0],
    };
  }
}
