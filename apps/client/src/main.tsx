...
import type { ActionCard, ChallengeTemplate, CheckIn, FeedItem, SessionState, TemplateCommunity, TemplateParticipant, TransformationReport, Visibility } from "@rawhabit/shared";
import "./styles.css";
...
function App() {
  ...
  const clone = async (feedItemId: string) => { try { setSession(await mutate(`/api/feed/${feedItemId}/clone`, { method: "POST" })); setResult(null); setReport(null); } catch (cause) { setError((cause as Error).message); } };
  ...
  return <><a className="skip-link" href="#main-content">Skip to feed</a><header className="topbar"><div className="topbar-inner"><a className="wordmark" href="/" aria-label="RawHabit home">raw<span>habit</span></a><nav aria-label="Primary navigation"><span className="nav-current">Home</span><span>Challenges</span><span>Graduates</span></nav><div className="profile-chip"><span className="avatar maya">M</span><span>{session?.user.status === "graduate" ? "Graduate" : "Maya’s space"}</span></div></div></header><main className="shell" id="main-content">
    ...
    <div className="home-grid"><section className="feed-column"><div className="feed-intro"><p className="eyebrow">The honest feed</p><h1>Progress is better when it’s <em>seen.</em></h1><p>Small reports from people doing the work, in public and without the highlight reel.</p></div><Feed feed={feed} onClone={clone} /></section><aside className="habit-rail" aria-label="Your habit space">
      ...
      {session?.activeChallenge && template && session.user.status === "challenger" && <><ChallengeDashboard template={template} session={session} onComplete={complete} onCommunity={openCommunity} /><Recorder onSubmit={submit} /><CoachResult result={result} onFeedback={sendFeedback} />{session.activeActionCard && <ActionCardView card={session.activeActionCard} onComplete={completeActionCard} />}</>}
...
function ChallengeDashboard({ template, session, onComplete, onCommunity }: { template: ChallengeTemplate; session: SessionState; onComplete: () => void; onCommunity: (id: string) => void }) { const day = session.activeChallenge!.currentDay; const percentage = Math.round((day / template.totalDays) * 100); const initiator = session.activeChallenge?.initiatedBy; return <section className="active-habit"><p className="eyebrow">RawHabit Official · Your active habit</p><h2>{template.title}</h2><div className="progress-label"><span>Day {day} of {template.totalDays}</span><span>{percentage}%</span></div><div className="progress" aria-label={`${percentage}% complete`}><i style={{ width: `${percentage}%` }} /></div>{initiator && <p className="lineage">Initiated by {initiator.displayName}</p>}<button className="community-link dark" onClick={() => onCommunity(template.id)}>◌ People building this habit</button><p className="rules">Today’s backup: {template.strategyRules[0]}</p><div className="demo"><span>Demo tools</span><button className="secondary" onClick={onComplete}>Complete challenge</button></div></section>; }
