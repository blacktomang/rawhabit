...
import type { ActionCard, ChallengeTemplate, CheckIn, CheckInJob, EarnedBadge, FeedItem, HabitProtocol, SessionState, TemplateCommunity, TemplateParticipant, TransformationReport, Visibility } from "@rawhabit/shared";
...
export function App() {
  ...
  return <><a className="skip-link" href="#main-content">Skip to today’s check-in</a><header className="topbar"><div className="topbar-inner"><a className="wordmark" href="/" aria-label="RawHabit home">raw<span>habit</span></a><nav aria-label="Primary navigation"><a className="nav-current" href="#main-content">Today</a><a href="#community">Community</a></nav>{session && <ProfileBadges displayName={session.user.displayName} badges={session.user.earnedBadges} />}</div></header><main className="shell" id="main-content">{error && <p className="alert">{error}</p>}<div className="home-grid"><aside className="habit-rail" aria-label="Your habit space">{!session?.activeChallenge && <TemplatePicker templates={templates} onStart={start} onCommunity={openCommunity} />}{session?.activeChallenge && template && session.user.status === "challenger" && <>{!session.habitProtocol || editingProtocol ? <HabitProtocolSetup template={template} protocol={session.habitProtocol} onSave={saveProtocol} /> : <><div className="today-intro"><p className="eyebrow">Today · Day {session.activeChallenge.currentDay} of {template.totalDays}</p><h1>Make today <em>honest.</em></h1><p>A private check-in is enough. You decide what, if anything, becomes public.</p></div><Recorder onSubmit={submit} processingStage={processingStage} /><ChallengeDashboard template={template} session={session} onComplete={completeChallenge} onCommunity={openCommunity} onEditProtocol={() => setEditingProtocol(true)} /><CoachResult result={result} feedbackMessage={feedbackMessage} onFeedback={sendFeedback} onPublish={publishCheckIn} onAdjust={() => setEditingProtocol(true)} />{session.pendingAgentActions.map((action) => <AgentProposal key={action.id} action={action} onResolve={resolveProposal} />)}{session.activeActionCard && <ActionCardView card={session.activeActionCard} onComplete={completeActionCard} />}</>}</>}{session?.user.status === "graduate" && template && <Graduate template={template} report={report} onReport={generateReport} onPost={victory} />}<p className="rail-note">RawHabit is for support, not diagnosis. If you’re in immediate danger, contact local emergency support.</p></aside><section className="feed-column" id="community"><div className="feed-intro"><p className="eyebrow">From the community</p><h2>People doing the work, too.</h2><p>Progress shared on their terms. Your raw check-in stays private unless you choose otherwise.</p></div><Feed feed={feed} onClone={clone} /></section></div>{community && <CommunitySheet community={community.data} participants={community.participants} onClose={() => setCommunity(null)} />}</main></>;
}

function ProfileBadges({ displayName, badges }: { displayName: string; badges: EarnedBadge[] }) { const featured = badges[0]; return <button className="profile-chip" type="button" aria-label={`${displayName}'s earned badges`}><span className="avatar maya">{displayName.slice(0, 1)}</span><span>{featured ? badgeLabel(featured.kind) : `${displayName}’s space`}</span><span className="badge-tooltip" role="tooltip"><strong>Earned badges</strong>{badges.length ? badges.map((badge) => <span key={badge.id}><b>{badgeLabel(badge.kind)}</b> · {badge.challengeTitle}</span>) : <span>Complete a challenge to earn your first badge.</span>}</span></button>; }
function badgeLabel(kind: EarnedBadge["kind"]) { return kind === "habit_builder" ? "Habit Builder" : "Pattern Breaker"; }
