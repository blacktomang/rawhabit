...
import { AgentProposal } from "../features/checkins/components/AgentProposal";
import { CoachResult } from "../features/checkins/components/CoachResult";
import { JourneySheet } from "../features/checkins/components/JourneySheet";
...
export function App() {
  ...
  const [journey, setJourney] = useState<CheckIn[] | null>(null);
  ...
  async function advanceDay() { try { setError(""); setSession(await mutate<SessionState>("/api/challenge/advance-day", { method: "POST" })); setResult(null); setFeedbackMessage(""); } catch (cause) { setError((cause as Error).message); } }
  async function reviewJourney() { try { setError(""); setJourney(await getJson<CheckIn[]>("/api/challenge/journey")); } catch (cause) { setError((cause as Error).message); } }
  ...
  async function resolveProposal(id: string, accepted: boolean) { try { setError(""); await mutate(`/api/agent-actions/${id}/resolve`, json({ accepted })); setSession((current) => current ? { ...current, pendingAgentActions: current.pendingAgentActions.filter((action) => action.id !== id), user: accepted ? { ...current.user, encouragementWelcome: current.pendingAgentActions.find((action) => action.id === id)?.kind === "request_encouragement" || current.user.encouragementWelcome } : current.user } : current); } catch (cause) { setError((cause as Error).message); } }
  ...
  return <><a className="skip-link" href="#main-content">Skip to today’s check-in</a><header className="topbar"><div className="topbar-inner"><a className="wordmark" href="/" aria-label="RawHabit home">raw<span>habit</span></a><nav aria-label="Primary navigation"><a className="nav-current" href="#main-content">Today</a><a href="#community">Community</a></nav>{session && <div className="profile-menu-anchor"><button className="profile-chip" type="button" aria-expanded={profileOpen} aria-controls="profile-menu" onClick={() => setProfileOpen((open) => !open)}><span className="avatar maya">{session.user.displayName.slice(0, 1)}</span><span>{session.user.earnedBadges.length ? `${session.user.earnedBadges.length} badge${session.user.earnedBadges.length === 1 ? "" : "s"}` : `${session.user.displayName}’s space`}</span><span aria-hidden="true">⌄</span></button>{profileOpen && <ProfileSheet displayName={session.user.displayName} badges={session.user.earnedBadges} onClose={() => setProfileOpen(false)} />}</div>}</div></header><main className="shell" id="main-content">{error && <p className="alert">{error}</p>}<div className="home-grid"><aside className="habit-rail" aria-label="Your habit space">{!session?.activeChallenge && <TemplatePicker templates={templates} onStart={start} onCommunity={openCommunity} />}{session?.activeChallenge && template && session.user.status === "challenger" && <>{!session.habitProtocol || editingProtocol ? <HabitProtocolSetup template={template} protocol={session.habitProtocol} onSave={saveProtocol} /> : <><div className="today-intro"><p className="eyebrow">Today · Day {session.activeChallenge.currentDay} of {template.totalDays}</p><h1>Make today <em>honest.</em></h1><p>A private check-in is enough. You decide what, if anything, becomes public.</p></div>{session.activeActionCard && <ActionCardView card={session.activeActionCard} onComplete={completeActionCard} />}{session.pendingAgentActions.map((action) => <AgentProposal key={action.id} action={action} onResolve={resolveProposal} />)}<Recorder onSubmit={submit} processingStage={processingStage} /><ChallengeDashboard template={template} session={session} onAdvanceDay={advanceDay} onReviewJourney={reviewJourney} onCommunity={openCommunity} onEditProtocol={() => setEditingProtocol(true)} /><CoachResult result={result} feedbackMessage={feedbackMessage} onFeedback={sendFeedback} onPublish={publishCheckIn} onAdjust={() => setEditingProtocol(true)} /></>}</>}{session?.user.status === "graduate" && template && <Graduate template={template} report={report} onReport={generateReport} onPost={victory} />}<p className="rail-note">RawHabit is for support, not diagnosis. If you’re in immediate danger, contact local emergency support.</p></aside><section className="feed-column" id="community"><div className="feed-intro"><p className="eyebrow">From the community</p><h2>People doing the work, too.</h2><p>Progress shared on their terms. Your raw check-in stays private unless you choose otherwise.</p></div><Feed feed={feed} onClone={clone} /></section></div>{community && <CommunitySheet community={community.data} participants={community.participants} onClose={() => setCommunity(null)} />}{journey && <JourneySheet checkIns={journey} onClose={() => setJourney(null)} />}</main></>;
}
